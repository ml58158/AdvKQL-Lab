...
def generate_event(ts):
    record = OrderedDict()
    action_type = random.choice(["RegistryValueSet", "RegistryKeyCreated", "RegistryValueDeleted", "RegistryKeyDeleted"])
    ...
    if "Deleted" not in action_type:
        record["RegistryValueData"] = "new_value"
    else:
        record["RegistryValueData"] = ""
    ...
    # Attack-aligned AdditionalFields
    record["AdditionalFields"] = json.dumps({
        "PersistenceTechnique": "StartupFolder",
        "RegistryChangeContext": "ScheduledTaskInjection",
